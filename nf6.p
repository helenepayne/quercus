program nf6 (input, output);

{10-21-93: Family structure out of first derivatives}
{11-18-93: New Numbering}
{11-29-93: New Family Structure}
{11-30-93: Matrix inversion added to Report to get correct large scale v-cv
 matrix in event of feasibility constraints imposed}
{12-9-93: DVA etc local to calcnewest; First removed; etc}
{1-6-94: Formal Active Set}
{1-10-94: integer values for EMPK derivative matrices on superb}
{5-19-94: repair work on reading in of fixed effects to avoid fe on missing records}
{5-25-94: factor of 2 in large sample v-cv output}
{6-3-94: up to speed with nf3}
{7-8-94: starting value option}
{9-7-94: inbred founders option}
{1-4-95: new ComputeBig. Decalaration of I in Getphtypes}
{2-3-95: fix bug in renumbering routine}
{2-15-95: fix error in initial values}
...
const r=1; {r=the max. number of traits}
      ...
      emaxnparm=30; {the max. number of params incl lagrange multipliers}
      f=1;  {the max number of families}
      sibs=120; {max # of individuals in a family, including unmeasured}
      rhsibs=52;
      fsibs=4; {max # of full sibs in a family}
      hsibs=26; {# of maternal (or paternal) sibs in family}
      rfsibs=4; {max # of recip full sibs in a family}
      missing=-98; {Value in the dataset indicating missing phenotypic data}
      pos=120; {largest number of observations in a family (=< r*sibs)}
      nmax=120; {maximum total # of individuals in the dataset, including
                 those missing data}
      epsilon = 0.001;  {Convergence criterion}
      maxround=10;
      nf = 1;  {the max. number of fixed effects}
      p = 6;  {the maximum number of levels of fixed effects=r~nlevels}
      nconstr = 12; {maximum number of constraint equations to keep track of}
      ifound = 0; {0 for noninbred founders, 1 for fully inbred founders}
 ...
 type list = array[1..nf] of integer;
     ...
     mmatrix = array[1..hsibs] of ICovrow;
     kmatrix = array[1..fsibs] of ICovrow;
     ...
     derivsm = array[1..nvarcov] of mmatrix;
     derivsk = array[1..nvarcov] of kmatrix;
     ...
     sextet = array[1..6] of real; {six component program}
     isextet = array[1..6] of integer; {six component program}
...
var  PresentData: Dataset;
     ...
     AddCov, DomCov, EnvCov, MatCov, PatCov, KCov :Paramvar;
     D, SumD, Parms:Parmvec;
     eD : eParmvec;
     Zeroed : Parmbool;
     better, alldone, llpos, restart: boolean;
     Done, REML, DomA, DomE, ok, Maxed, Trunked: boolean;
...
procedure CoeffMatrices (var Data: Dataset; var Fm: Famarray; 
                         var coeff: sextet; I, L, K: integer; var DomA, DomE: boolean);
   ...
   var valA, valD, valE, valM, valP, valK: real;
   ...
   begin {CoeffMatrices}
	 ...
	 valK := 0;
	    ...
	    if SameFather
	    then valP := 1.0;
            if ifound = 1 then valA := 0.5 else
            valA:=0.25;
            if (SameMother and SameFather) or (Opposite1 and Opposite2)
            then begin
             if ifound = 1 then valA := 1.0 else
               valA:= 0.5;
             if ifound = 1 then valD := 1.0 else
               valD:=0.25;
	       if (SameMother and SameFather)
	       then valK := 1.0;
               end;
...
procedure EnterValues(var AddCov, DomCov, EnvCov, MatCov, PatCov, 
			  KCov :Paramvar);
   ...
   begin
      ...
      for I:=1 to nchar do begin
         for J:=I to nchar do begin
            ...
            loc := loc + group;
	    MatCov[I,J]:= Parms[loc];
	    MatCov[J,I] := MatCov[I,J];
            loc := loc + group;
	    PatCov[I,J]:= Parms[loc];
	    PatCov[J,I] := PatCov[I,J];
            loc := loc + group;
	    KCov[I,J]:= Parms[loc];
	    KCov[J,I] := KCov[I,J];
            end; {J loop}
...
procedure MakeCovArray (var EstCovArray:Covarray; var DomA, DomE: boolean);
                  ...
                  for J:= 1 to famsize[G] do begin
                     for L:= 1 to nchar do begin
                        ...
                        then skippedj := skippedj + 1
                           ...
                           CoeffMatrices(PresentData,Fm,coeff,G,I,J,DomA,DomE);
...
procedure VMatrixInversion (var A:Covmatrix; var Ldet: real; n: integer;
   ...
   procedure Rootit (var A: Covmatrix; var Ldet: real; var ok: boolean);
     ...
     writeln ('The Variance Matrix is not positive definite for these estimates');
          printparms;
          halt;
         end;
         Ldet := 0;
...
procedure sMatrixInversion (var A:Traitsquare; var Ldet: real; n: integer;
   ...
   procedure Correction (var Rfix: Traitsquare; var Des:Desmatrix);
...
procedure CalcNewEsts (var Parms: Parmvec; var lln: lagvec; 
 ...
 var length, nvarparms, totvar : integer;
     ...
     tally : real;
     tdva, tdvd, tdve, tdvm, tdvp, tdvk: countset;
     DVA : derivsa;
     DVD : derivsd;
     DVM, DVP : derivsm;
     DVK : derivsk;
...
procedure DVprods (var DVA:derivsa; var DVD:derivsd;
			      var DVE:derivse; var DVM, DVP:derivsm;
			      var DVK: derivsk;
			      var tdva, tdvd, tdve, tdvm, tdvp, tdvk: countset; 
     ...
     if ((not Trunked) or (not Zeroed[g+3*nvc])) then begin
	    for i := 1 to length do begin
	       tally := 0.0;
	       for s := 1 to slevels do
                  t[s] := 0;
               for j := 1 to tdvm[g][i] do begin
		  index := DVM[g][j][i] ;
		  tally := tally + Psub[index];
		  if REML 
		  then begin
		    for s := 1 to slevels do
             ...
             D[g+3*nvc] := D[g+3*nvc] + Psub[i] * tally;
	     if REML
	     then begin
		for s := 1 to slevels do
   	           CU[g+3*nvc][i][s] := t[s];
     ...
     if ((not Trunked) or (not Zeroed[g+4*nvc])) then begin
	    for i := 1 to length do begin
	       tally := 0.0;
	       for s := 1 to slevels do
                  t[s] := 0;
               for j := 1 to tdvp[g][i] do begin
		  index := DVP[g][j][i] ;
		  tally := tally + Psub[index];
		  if REML 
		  then begin
		    for s := 1 to slevels do
             ...
             D[g+4*nvc] := D[g+4*nvc] + Psub[i] * tally;
	     if REML
	     then begin
		for s := 1 to slevels do
   	           CU[g+4*nvc][i][s] := t[s];
     ...
     if ((not Trunked) or (not Zeroed[g+5*nvc])) then begin
	    for i := 1 to length do begin
	       tally := 0.0;
	       for s := 1 to slevels do
                  t[s] := 0;
               for j := 1 to tdvk[g][i] do begin
		  index := DVK[g][j][i] ;
		  tally := tally + Psub[index];
		  if REML 
		  then begin
		    for s := 1 to slevels do
             ...
             D[g+5*nvc] := D[g+5*nvc] + Psub[i] * tally;
	     if REML
	     then begin
		for s := 1 to slevels do
   	           CU[g+5*nvc][i][s] := t[s];
...
procedure MakeFirsts (var DVA:derivsa; var DVD:derivsd;
                              var DVE:derivse; var DVM, DVP:derivsm;
                              var DVK: derivsk;
                              var tdva, tdvd, tdve, tdvm, tdvp, tdvk:
                              countset; var G: integer);
   ...
   procedure ComputeBig (var Q: Covmatrix; var B: info; length, totvar:integer);
	...
	for i := 1 to length do begin
	   ...
	   for j := 1 to nvc do begin
        ...
        if ((not Trunked) or (not Zeroed[j+3*nvc])) then begin
		 tally := 0.0;
		 for l := 1 to tdvm[j][k] do begin
                    index := DVM[j][l][k];
		    tally := tally + Q[i][index];
		    end; {l}
                 QGI[j+3*nvc,k] := tally;
        end; {if not Trunked}
        if ((not Trunked) or (not Zeroed[j+4*nvc])) then begin
		 tally := 0.0;
		 for l := 1 to tdvp[j][k] do begin
                    index := DVP[j][l][k];
		    tally := tally + Q[i][index];
		    end; {l}
                 QGI[j+4*nvc,k] := tally;
        end; {if not Trunked}
        if ((not Trunked) or (not Zeroed[j+5*nvc])) then begin
		 tally := 0.0;
		 for l := 1 to tdvk[j][k] do begin
                    index := DVK[j][l][k];
		    tally := tally + Q[i][index];
		    end; {l}
                 QGI[j+5*nvc,k] := tally;
        ...
        if ((not Trunked) or (not Zeroed[j+3*nvc])) then begin
		 for m := 1 to tdvm[j][i] do begin
		    index := DVM[j][m][i];
		    for k := i to length do
		       QHI[j+3*nvc,k] := QHI[j+3*nvc,k] + Q[k][index];
		    end; {m}
        end; {if not Trunked}
        if ((not Trunked) or (not Zeroed[j+4*nvc])) then begin
		 for m := 1 to tdvp[j][i] do begin
		    index := DVP[j][m][i];
		    for k := i to length do
		       QHI[j+4*nvc,k] := QHI[j+4*nvc,k] + Q[k][index];
		    end; {m}
        end; {if not Trunked}
        if ((not Trunked) or (not Zeroed[j+5*nvc])) then begin
		 for m := 1 to tdvk[j][i] do begin
		    index := DVK[j][m][i];
		    for k := i to length do
		       QHI[j+5*nvc,k] := QHI[j+5*nvc,k] + Q[k][index];
	            end; {m}
        end; {if not Trunked}
		 end;  {j}
...
begin {Gauss-Jordan reduction -- invert
 ...
 begin {CalcNewEsts}
    ...
    for fcount := 1 to famnum do begin
       ...
       MakeFirsts (DVA,DVD,DVE,DVM,DVP,DVK,
                 tdva,tdvd,tdve,tdvm,tdvp,tdvk,fc);
       DVprods (DVA,DVD,DVE,DVM,DVP,DVK,
                     tdva,tdvd,tdve,tdvm,tdvp,tdvk,length,Py[fcount],
                     D,CU,AB);
       Big := Empty;
...
procedure Report;
   ...
   begin
    ...
    writeln (Analysis);
    writeln (Analysis, '   Higher Interactions');
    writeln (Analysis,'              ');
    for i := 1 to nchar do begin
       for j := i to nchar do begin
          write (Analysis, Parms[l]:14:6);
...
{Vax initialization} 
  ...
  rewrite ( Analysis );} {VAX initialization}
  if ifound = 1 then writeln ('Fully inbred founders assumed');
   ncut := 0;
   tncut := 0;
   Iter := 0;
   Done := false;
  ...
  repeat
   repeat
    if not restart then begin
         EnterValues (AddCov, DomCov, EnvCov, MatCov, PatCov, KCov);
         MakeCovArray (Q,DomA,DomE);
         Iter := Iter + 1;
         vartypes := 6;
         better:=true;
	 ok := true;
