program pcrf1 (input, output);

 {from nf3.p 5-26-94 to do pop comps}
 {1-4-95 new ComputeBig and corrected Getphtypes}
 {2-3-95 fixe bug in numbering code}

const r=1; {r=the max. number of traits}
      nvarcov = 1; {the number of (co)variances of a given type; r*(r+1)/2}
      maxnparm=6; {the max. number of params}  {incl., Mat, Pat, k}
      emaxnparm=9; {the max. number of params incl lagrange multipliers}
      f=2;  {the max number of families}
      sibs=270; {max # of individuals in a family, including unmeasured}
      rhsibs=105;
      rfsibs=12; {max # of recip full sibs in a family}
      missing=-98; {Value in the dataset indicating missing phenotypic data}
      pos=260; {largest number of observations in a family (=< r*sibs)}
      nmax=631; {maximum total # of individuals in the dataset, including
                 those missing data}
      epsilon = 0.00001;  {Convergence criterion}
      maxround=20;
      df = 2;  {the max. number of fixed effects}
      p = 8;  {the maximum number of levels of fixed effects=r~nlevels}
      ceo = 1; {common environment option..4 for c.e, 1 for dominance}
      nconstr = 5; {maximum number of constraint equations to keep track of}



 type list = array[1..df] of integer;
      Observation=record
                   ID:integer;
                   Father, Mother, popid: integer;
     ...
     items = array[1..df,1..p] of integer;
     ...
     isextet = array[1..6] of integer; {2 pops x three component program}
...
var  PresentData: Dataset;
     ...
     AddCov1, DomCov1, EnvCov1:Paramvar;
     AddCov2, DomCov2, EnvCov2:Paramvar;
     D, SumD, Parms, First:Parmvec;
     eD : eParmvec;
     Zeroed, Nullhyp: Parmbool;
     better, alldone, llpos, restart: boolean;
     Done, REML, ok, Maxed, Trunked: boolean;
     bfl, feasible, afeasible: boolean;
     flagvec: slevelsbool;
     pcdata: text;
     pcout: text;
     nchar, ntrunc, slevels, nfxfact :integer;
     newfam, I, J, Iter, ncut, tncut, nsame, nvc: integer;
     Fm: Famarray;
     AB, VInvX: Famphens;
     Py: FamVector;
...
procedure Getphtypes(var Data:Dataset; var M: integer; var nchar:integer;
   ...
   begin
      ...
      while not (TempID= 0) do begin
         ...
         read (pcdata, TempFather,TempMother);
	    ...
	    Mother:=TempMother;
          ...
          with Data[M] do begin
            for K := 1 to nfxfact do begin
               ...
               then begin
                  ...
                  write (pcout, which,'is fixed effect number',nfxlevels[K]);
                  writeln (pcout, 'of factor ', K);
                  slevels := slevels + 1;
         ...
         else begin {take care of popid here even if no obs}
          with Data[M] do begin
               which := FixedVals[1];
               already := false;
               count := 0;
               for L := 1 to nfxlevels[1] do begin
                  count := count + 1;
                  if (which=levels[1,L])
                  then begin
                    already := true;
               ...
               if not already
               then begin
                  nfxlevels[1]:= nfxlevels[1] + 1;
                  FixedVals[1]:= nfxlevels[1];
                  write (pcout, which,'is fixed effect number',nfxlevels[1]);
                  writeln (pcout, 'of factor ', 1);
                  slevels := slevels + 1;
                  levels[1,nfxlevels[1]] := which;
                end;
            popid := FixedVals[1];
            end; {with}
         end; {not an observation}
         readln (pcdata);
         read (pcdata,TempID);
      end; {while not}
   readln (pcdata);
  ...
  {reset the parental id numbers positive}
   ...
   slevels := nchar * slevels;
     writeln ('The number of fixed effects is',slevels:5);
...
{procedure PrintFams (var Data: Dataset; Zmatrix:Measures);
   ...
   var I, J: integer;
   begin
  writeln (pcout, 'Loc ID   ID  Father Mother NPS  NMS  Famid       Observations  ');
      for I := 1 to M do begin
         with Data[I] do begin
            write (pcout, I:5, ID:5, Father:7, Mother:7, NPS:5);
            write (pcout, NMS:5, Famid:5);
            end;
            for J:= 1 to nchar do
	      write (pcout, Zmatrix[I,J]:8:2);
              writeln(pcout);
      end;
   end;} {PrintFams}
...
procedure EnterValues(var AddCov1, AddCov2, DomCov1, DomCov2,
                          EnvCov1, EnvCov2 :Paramvar);
      ...
      begin
         ...
         for I:=1 to nchar do begin
            for J:=I to nchar do begin
               base := base + 1;
               loc := base;
               AddCov1[I,J]:= Parms[loc];
               AddCov1[J,I] := AddCov1[I,J];
               loc := loc + group;
               AddCov2[I,J] := Parms[loc];
               AddCov2[J,I] := AddCov2[I,J];
               end; {J}
            end; {I}
         base := 2 * group;
   writeln (pcout);
   writeln (pcout,' At iteration ',Iter:5);
   writeln (pcout);
   writeln (pcout,' The current parameter values:  ');
   writeln (pcout,' Additive, population 1:  ');
         for I := 1 to nchar do begin
            for J := 1 to nchar do
               write (pcout,AddCov1[I,J]:6:3);
            writeln (pcout);
            end;
   writeln (pcout,' Additive, population 2:  ');
         for I := 1 to nchar do begin
            for J := 1 to nchar do
               write (pcout,AddCov2[I,J]:6:3);
            writeln(pcout);
            end;
         for I := 1 to nchar do begin
            for J:=I to nchar do begin
               base := base + 1;
               loc := base;
               EnvCov1[I,J]:= Parms[loc];
               EnvCov1[J,I]:=EnvCov1[I,J];
               loc := loc + group;
               EnvCov2[I,J]:= Parms[loc];
               EnvCov2[J,I]:=EnvCov2[I,J];
               loc := loc + group;
               DomCov1[I,J] := Parms[loc];
               DomCov1[J,I] := DomCov1[I,J];
               loc := loc + group;
               DomCov2[I,J] := Parms[loc];
               DomCov2[J,I] := DomCov2[I,J];
               end; {J loop}
            end;
   writeln (pcout,' Environmental, population 1:  ');
         for I := 1 to nchar do begin
            for J := 1 to nchar do
               write (pcout,EnvCov1[I,J]:6:3);
            writeln(pcout);
            end;
   writeln (pcout,' Environmental, population 2:  ');
         for I := 1 to nchar do begin
            for J := 1 to nchar do
               write (pcout,EnvCov2[I,J]:6:3);
            writeln(pcout);
            end;
   writeln (pcout,' Dominance, population 1:  ');
         for I := 1 to nchar do begin
            for J := 1 to nchar do
               write (pcout,DomCov1[I,J]:6:3);
            writeln(pcout);
            end;
   writeln (pcout,' Dominance, population 2:  ');
         for I := 1 to nchar do begin
            for J := 1 to nchar do
               write (pcout,DomCov2[I,J]:6:3);
            writeln(pcout);
            end;
         end;   {EnterValues}
...
procedure MakeCovArray (var EstCovArray:Covarray);
 ...
 {The V matrix is assembled.}
   ...
   begin
      for G:= 1 to famnum do begin
            if PresentData[Fm[G][1]].popid=1
            then begin
               DomCov := DomCov1;
               EnvCov := EnvCov1;
               AddCov := AddCov1;
               end;
            if PresentData[Fm[G][1]].popid=2
            then begin
               DomCov := DomCov2;
               EnvCov := EnvCov2;
               AddCov := AddCov2;
   ...
   procedure Rootit (var A: Covmatrix; var Ldet: real; var ok: boolean);
     {The square root.}
      ...
      begin
        ...
        then begin
	   ok := false;
           writeln (pcout,'The variance matrix is not pos. def.');
	   end
        else if ok
         then begin
           A[1][1] := sqrt(A[1][1]);
...
procedure sMatrixInversion (var A:Traitsquare; var Ldet: real; n: integer;
   ...
   procedure sRootit (var A: Traitsquare; var Ldet: real; var ok: boolean);
     {The square root.}
      ...
      begin
        ...
        then begin
	   ok := false;
           writeln (pcout,'The XVX matrix is not pos. def.');
	   end
        else begin
           A[1][1] := sqrt(A[1][1]);
           for i := 2 to n do
   ...
   begin {ComputeLogLikelihood}
      ...
      ok := true;
      for g:=1 to famnum do begin
         if ok then VMatrixInversion (Q[g],Ldet, vert[g], ok);
         if ok then begin
     ...
     writeln (pcout,'LogLike ', LogLike:15:4);
...
procedure CalcNewEsts (var Parms: Parmvec; var lln: lagvec; 
 ...
 var length, nvarparms, totvar : integer;
     fcount, fc, i, j, k, s, t, pop: integer;
     tally : real;
...
procedure DVprods (var DVA:derivsa; var DVD:derivsd; var DVE:derivse;
	 ...
	 for g := 1 to totvar do
	    D[g] := 0.0;
         ...
         if pop = 1 then ghat := g else ghat := nvc + g;
     if ((not Trunked) or (not Zeroed[ghat])) then begin
		...
		for s := 1 to slevels do
         ...
         if pop = 1 then ghat := 2 * nvc + g else ghat := 3 * nvc + g;
     if ((not Trunked) or (not Zeroed[ghat])) then begin
	    for i := 1 to length do begin
	       tally := 0.0;
	       for s := 1 to slevels do
		...
		for s := 1 to slevels do
         ...
         if pop = 1 then ghat := 4 * nvc + g else ghat := 5 * nvc + g;
     if ((not Trunked) or (not Zeroed[ghat])) then begin
	    for i := 1 to length do begin
	       tally := 0.0;
	       for s := 1 to slevels do
...
procedure MakeFirsts (var DVA:derivsa; var DVD:derivsd; var DVE:derivse;
 ...
 {Compute family G's derivative matrices.  The coefficients for the A and D
  ...
  desired}
   ...
   procedure ComputeBig (var Q: Covmatrix; var B: info; length, totvar, pop:integer);
      ...
      var g, h, i, j, k, l, m, index, nvc, jhat: integer;
        tally, fact: real;
	  QGI, QHI: linemat;
      begin
        nvc := (nchar * (nchar + 1)) div 2;
	for i := 1 to length do begin
	   ...
	   for j := 1 to nvc do begin
		 ...
		 tally := 0.0;
        if pop = 1 then jhat := j else jhat := nvc + j;
        if ((not Trunked) or (not Zeroed[jhat])) then begin
        ...
        if pop = 1 then jhat := 4 * nvc + j else jhat := 5 * nvc + j;
        if ((not Trunked) or (not Zeroed[jhat])) then begin
		 tally := 0.0;
        ...
        if ((not Trunked) or (not Zeroed[jhat])) then begin
		 for m := 1 to tdva[j][i] do begin
		    ...
		    for k := i to length do
		       QHI[jhat,k] := QHI[jhat,k] + Q[k][index]*fact;
		    end; {m}
        end; {if not Trunked}
        if pop = 1 then jhat := 4 * nvc + j else jhat := 5 * nvc + j;
        if ((not Trunked) or (not Zeroed[jhat])) then begin
...
begin {Gauss-Jordan reduction -- invert
   ...
   procedure eNewtheta (eD: eParmvec; var eB: einfo; 
      ...
      begin
         ...
         for i := 1 to ncut do begin
           ...
           writeln (pcout,'Multiplier',aset[i]:3,lln[aset[i]]:8:4);
          end; {i}
          end;
 ...
 begin {CalcNewEsts}
    ...
    for fcount := 1 to famnum do begin
       ...
       DVprods (DVA,DVD,DVE,tdva,tdvd,tdve,length,Py[fcount],D,CU,AB,pop);
       Big := Empty;
       ComputeBig (Q[fcount],Big, length, totvar, pop);
       for i := 1 to totvar do begin
        if ((not Trunked) or (not Zeroed[i])) then begin
          SumD[i] := D[i] + SumD[i];
          for j := 1 to totvar do begin
...
procedure CuttingPlane (var Parms: Parmvec; var cvec: info; var ncut: integer;
                        var lln: lagvec; var aset: active; 
                         var alldone: boolean; var cfactors: isextet);
...
{make the matrix up}
 ...
 writeln;
 if abs(mineig/trace) < 0.00005 then begin
  alldone := true;
  writeln ('v-cv matrix is (at least real close to) pos def ');
 for i := nsame + 1 to ncut do begin
   writeln (pcout, 'Constraint equation number ',i:5);
   for j := 1 to mnp do begin
    write (pcout, cvec[j,aset[i]]:10:6);
    if j mod 5 = 0 then writeln (pcout);
   end;
   writeln (pcout);
  end; {i}
  end {pos def}
...
{deal with the case that the new cutting plane overconstrains a random factor}
 ...
 minind := 0;
 mintrace := 1.0;
 for j := nsame + 1 to ncut do begin
  trace := 0.0;
  for i := 1 to mnp do
...
procedure Test ( Old: real; Current: real; var Maxed, better: boolean);
   ...
   and last loglikelihood to delta, the stopping criterion, to determine
   ...
   begin
      ...
      then better := true;
      diff := abs(diff);
      if diff < epsilon * abs(Old) then Maxed := true
       else Maxed := false;
      end;
...
procedure Report;
   ...
   begin
    nvc := (nchar* (nchar + 1)) div 2;
    totvar := vartypes * nvc;
    writeln (pcout,'    At iteration ',Iter:4);
    writeln (pcout);
    if Trunked or (ncut > 0)
    then write (pcout,'   *** The constrained analysis converged ')
    else write (pcout,'   *** The unconstrained analysis converged ');
    writeln (pcout,'with the following results ***');
    writeln (pcout,'   The log likelihood is ', LogLike:8:4);
    writeln (pcout,'   The mean of each trait is');
    for i := 1 to nchar do
       writeln (pcout, '   ',Means[i]:14:6);
    writeln (pcout,'   The effect of the fixed factors is');
    writeln (pcout,'   (in the order given, levels within factors)');
     count := nchar;
     ...
     then begin
     writeln (pcout,'Factor: ',i:4,' Level: ',j:4,' Label: ',levels[i,j]:4);
         writeln (pcout,'has fixed factor effect  0');
         end
     else begin
        count := count + 1;
        writeln (pcout,'Factor: ',i:4,' Level: ',j:4);
        writeln (pcout,' Label: ',levels[i,j]:4);
         writeln (pcout,'has fixed factor effect  ',Means[count]:14:6);
         end;
       end; {k}
     end; {j}
   end; {i}
    writeln (pcout);
         k := 0;
         writeln (pcout,'additive genetic components for population 1');
         for i := 1 to nchar do begin
            for j := 1 to (i-1) do
               write (pcout,'          ');
            for j := i to nchar do begin
               k := k + 1;
               write (pcout,Parms[k]:10:4)
               end;
            writeln (pcout);
            end;
         writeln (pcout);
         writeln (pcout,'additive genetic components for population 2');
         for i := 1 to nchar do begin
            for j := 1 to (i-1) do
               write (pcout,'          ');
            for j := i to nchar do begin
            ...
            writeln (pcout);
      for popnum := 1 to 2 do begin
         writeln (pcout,'environmental components for population ', popnum);
         for i := 1 to nchar do begin
            for j := 1 to (i-1) do
               write (pcout,'          ');
            for j := i to nchar do begin
      ...
      writeln (pcout);
      for popnum := 1 to 2 do begin
         writeln (pcout,'dominance components for population ', popnum);
         for i := 1 to nchar do begin
            for j := 1 to (i-1) do
               write (pcout,'          ');
               for j := i to nchar do begin
         ...
         writeln (pcout);
    writeln (pcout,'    Large-sample var-cov matrix of the estimates');
   ...
   for i := 1 to totvar do begin
       ...
       writeln (pcout);
       write (pcout,'  The test statistic comparing two likelihoods is ');
       writeln (pcout,'given by twice');
       write (pcout, '  their difference and is compared to ');
       writeln (pcout,'Chi-square with df given by the ');
       write (pcout, '  number of parameters ');
       writeln (pcout, 'specified by the hypothesis.');
    end;

procedure Cleanup (var hypo: Parmvec; var lln: lagvec;
                   var charnum: integer; var REML: boolean;
                   var ntrnc, nsame: integer; var tvec, Nullhyp: Parmbool; 
   ...
   var Analtype, feas: integer;
      nvc, totvar, i, j, k, index, stv: integer;
   begin
      for i := 1 to maxnparm do begin
	 tvec[i] := false;
         Nullhyp[i] := false;
      end;
      read (pcdata, Analtype, charnum, nfxfact, feas, stv);
      writeln (Analtype);
      ...
      index := 0;
      for k := 1 to 6 do begin {starting values give I for first V matrix}
       for i := 1 to nchar do begin
        for j := i to nchar do begin
         index := index + 1;
         if stv = 1 then read (pcdata,hypo[index])
         else begin
         if ((k = 3) or (k = 4)) and (i = j) then
          hypo[index] := 1
         else
          hypo[index] := 0;
         end;
      ...
      if REML
      then writeln (pcout,'  This is an REML analysis.')
      else writeln (pcout,'  This is an ML analysis.');
      if ceo = 1 
      ...
      read (pcdata,nsame);
      if nsame > 0 then begin
       writeln(' The following G-Matrix elements will be constrained ');
       write('  to be the same in both populations: ');
      for i := 1 to nsame do begin
         read (pcdata,index);
         write (index:4);
         Nullhyp[index] := true;
         end;
       ...
       writeln(' The G-Matrices will not be constrained. ');
      readln (pcdata);
      end;

procedure testkt (var lln: lagvec; var aset: active; var ncut: integer; 
                   var cvec: info; var llpos: boolean; var cfactors: isextet);
...
begin
 flag1 := false;
 flag2 := false;
 mnp := 6 * nchar * (nchar + 1) div 2; {in case maxnparm is set too high}
 llpos := true;
 {check inactive constraints, if any, to make sure they're slack}
 if ncut < tncut then begin
  minsum := 0.0;
  for i := nsame + 1 to tncut do begin
   sum := 0.0;
   flag := true;
   for j := 1 to ncut do
    if aset[j] = i then flag := false; {will eq constraints be in aset?}
  ...
  if minsum < -2.0 * epsilon then begin
   ncut := ncut + 1;
   aset[ncut] := minind;
...
procedure printparms;
   ...
   begin
        k := 0;
         ...
         writeln;
         writeln ('additive genetic components for population 2');
         for i := 1 to nchar do begin
            for j := 1 to (i-1) do
               write ('          ');
            for j := i to nchar do begin
            ...
            writeln;
      for popnum := 1 to 2 do begin
         writeln ('environmental components for population ', popnum);
         for i := 1 to nchar do begin
            for j := 1 to (i-1) do
               write ('          ');
            for j := i to nchar do begin
      ...
      writeln;
      for popnum := 1 to 2 do begin
         writeln ('dominance components for population ', popnum);
         for i := 1 to nchar do begin
            for j := 1 to (i-1) do
               write ('          ');
               for j := i to nchar do begin
...
begin {program nf3} 
  ...
  rewrite (pcout,'pcout');
   ...
   Maxed := false;
   nsame := 0;
   Cleanup (First, lln, nchar, REML, ntrunc, nsame,
            Zeroed, Nullhyp, nfxfact, afeasible);
   Parms := First;
   if afeasible then feasible := true;
   nvc := nchar * (nchar + 1) div 2;
   if (afeasible) or (nsame > 0) then feasible := true;
   ncut := 0;
   tncut := 0;
   if nsame > 0 then begin
    for I := 1 to nvc do begin
     if Nullhyp[I] then begin
      ncut := ncut + 1;
      tncut := tncut + 1;
      aset[ncut] := ncut;
  ...
  repeat
   repeat
    if not restart then begin
         EnterValues (AddCov1,AddCov2,DomCov1,DomCov2,EnvCov1, EnvCov2);
         MakeCovArray (Q);
         Iter := Iter + 1;
         vartypes := 6;
         better:=true;
     ...
     if ok then begin
      ...
      if ncut > nsame then begin
       writeln ('The program will begin with new starting values');
       Parms := First;
      end
      else begin
       writeln ('The program will terminate here.');
       writeln ('Look at these estimates and see if some can be constrained');
       printparms;
       halt;
      end;
      end; {not ok}
